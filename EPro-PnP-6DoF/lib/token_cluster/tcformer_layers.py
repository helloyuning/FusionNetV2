...
class TCBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = q_dict['x']
        # print("正常的形状:",q_dict['x'].shape,self.t)
        # norm1
...
class CTM(nn.Module):
    def __init__(self, sample_ratio, embed_dim, dim_out, k=5):
        ...
        # self.conv = TokenConv(in_channels=embed_dim, out_channels=dim_out, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, token_dict):
        token_dict = token_dict.copy()#[16,1024,256]
        # print("输入输出维度",self.t,self.f)
        x = self.conv(token_dict)
        # print("通过卷积",token_dict['x'].shape)
        x = self.norm(x)

        token_score = self.score(x)
        token_weight = token_score.exp()# pi密度
        ...
        #idx_cluster:[16, 1024], cluster_num:256
        idx_cluster, cluster_num = cluster_dpc_knn(
            token_dict, cluster_num, self.k)
        # print("cluster num",idx_cluster.shape)
