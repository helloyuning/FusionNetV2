...
class TCFormer(nn.Module):
    def __init__(
            #原来为224
            self, img_size=256, in_chans=3, embed_dims=[64, 128, 256, 512],
        ...
        self.down = nn.Conv2d(64, 304, 1, stride=8, bias=False)  # 我家的
    ...
    def forward_features(self, x):
        ...
        x, H, W = patch_embed(x)#[16, 4096, 64]
        print("当前的形状:",H, W)
        for blk in block:
            x = blk(x, H, W)
        x = norm(x)
        ...
        for i in range(1, self.num_stages):
            ...
            token_dict = ctm(token_dict)  # down sample[16, 4096, 64]
    ...
    def forward(self, x):
        ...
        #x[0]['x'] 形状：[16, 4096, 64]
        # print("输出:", x[0]['x'].shape)
        bs,h,w = x[0]['x'].shape
        x = x[0]['x'].permute(1, 2, 0).contiguous().view(bs, 64, 64, 64)
        x = self.down(x)
        # print("输出形状:",x.shape)
        return x
