...
from transformers import SwinModel #导入transformer
...
def train(epoch, cfg, data_loader, model, obj_info, criterions, optimizer=None, swin=None):
    ...
    for i, (obj, obj_id, inp, target, loss_msk, trans_local, pose, c_box, s_box, box) in enumerate(data_loader):
        ...
        # print("noc,w2d,scale的形状:",noc.shape,w2d.shape,scale.shape,"位移的形状:",pred_trans.shape)
        # print("inp",inp_var.shape)
        # swin_feature = swin(inp_var)
        # print("swin特征尺寸:", swin_feature.shape)
        #print("输出形状:",np.shape(noc),np.shape(w2d),np.shape(scale))
        #summary(model, (3,256,256)) #查看网络输出形状
