...
def token2map(token_dict):
    ...
    B, N, C = x.shape
    # print('H_init ,W_init',H_init,W_init)
    ...
    if N_init < N * H * W:
        ...
        # print("idx_token, idx_batch",idx_token.shape,idx_batch.shape)
...
def merge_tokens(token_dict, idx_cluster, cluster_num, token_weight=None):
    ...
    B, N, C = x.shape
    # print("开始合并：",x.shape)[16,1024,256]
    ...
    # print("合并后",x_merged.shape)
    source = x * norm_weight
    x_merged.index_add_(dim=0, index=idx.reshape(B * N),
                        source=source.reshape(B * N, C).type(x.dtype))
    x_merged = x_merged.reshape(B, cluster_num, C)
    # print("最终合并",x_merged.shape)
